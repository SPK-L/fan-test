...
struct AioCompletion {
  ...
  map<string ,utime_t> oid_latency_map;
...
public:
  ...
  map<string , utime_t> oid_latency;
  void insert_oid_latency(std::pair<string, utime_t> oid_info){
	oid_latency.insert(oid_info);
  }
 
  virtual ~C_AioRequest() {}
  virtual void finish(int r) {
  	utime_t now = ceph_clock_now(m_completion->ictx->cct);
  	map<string ,utime_t>::iterator iter = oid_latency.begin();
    for(; iter!=oid_latency.end(); iter++){
	  oid_latency[iter->first] = now - iter->second;
    }
	m_completion->oid_latency_map.insert(oid_latency.begin() ,oid_latency.end());
	oid_latency.clear();
