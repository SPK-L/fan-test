...
struct C_InvalidateCache : public Context {
  ...
  ImageCtx::ImageCtx(const string &image_name, const string &image_id,
    ...
    : cct((CephContext*)p.cct()),
      ...
      rbd_io_stat_lock(util::unique_lock_name("librbd::ImageCtx::rbd_io_stat_lock", this)),
      extra_read_flags(0),
      old_format(true),
      ...
      asok_hook(nullptr)
  {
    ...
    dump_ops_queue = new DumpOpsWQ(this, "librbd::dump_ops_queue");
  ...
  void ImageCtx::init() {
    ...
    std::string p_name = data_ctx.get_pool_name();
    set_rbd_io_stat_image_name(name);
    set_rbd_io_stat_pool_name(p_name);
    dump_ops_queue->start();
  }

  uint32_t ImageCtx::get_rbd_stat_io_op_stats_oid_size() {
  	return oid_num;
  }

  uint32_t ImageCtx::get_rbd_stat_io_op_stats_size() {
    uint32_t op_stats_size = ris->op_stats.size();
	return op_stats_size;
  }

  void ImageCtx::set_rbd_io_stat_image_name(std::string i_name) {
	 Mutex::Locker l(rbd_io_stat_lock);
	 ris->image_name = i_name;
  }
  void ImageCtx::set_rbd_io_stat_pool_name(std::string p_name) {
	 Mutex::Locker l(rbd_io_stat_lock);
	 ris->pool_name = p_name;
  }

  std::string ImageCtx::get_rbd_io_stat_image_name() {
	 return ris->image_name;
  }

  std::string ImageCtx::get_rbd_io_stat_pool_name() {
	 return ris->pool_name;
  }

  std::list<RbdOpStat> ImageCtx::get_rbd_io_stat_op_stats() {
	return ris->op_stats;
  }
  	
  bool ImageCtx::can_trim_rbd_io_stat_op(utime_t now ,double rbd_op_time_to_keep ,uint32_t max_op_size) {
    Mutex::Locker l(rbd_io_stat_lock);
	if( (get_rbd_stat_io_op_stats_size() && now - ris->op_stats.begin()->time > rbd_op_time_to_keep) ||
		 get_rbd_stat_io_op_stats_oid_size() > max_op_size )
	    return true;
	else
	    return false;
  }

  void ImageCtx::trim_rbd_io_stat_front_op() {
    Mutex::Locker l(rbd_io_stat_lock);
    if(get_rbd_stat_io_op_stats_size()) {
	   ldout(cct ,20) << "trim op stats over keep time front!" << "  " 
	   << "the time is : " << ris->op_stats.begin()->time << dendl;
	   
    	RbdOpStat ros = ris->op_stats.front();
	    ris->op_stats.erase(ris->op_stats.begin());
	    ldout(cct, 30) << "oid_num before trim_rbd_io_stat_front_op is " <<  oid_num << dendl;
	    oid_num -= ros.oid_latency.size();
	    ldout(cct, 30) << "oid_num after trim_rbd_io_stat_front_op is " <<  oid_num << dendl;
    }
  }

  void ImageCtx::insert_rbd_io_stat_op(RbdOpStat *ros) {
	Mutex::Locker l(rbd_io_stat_lock);
	ris->op_stats.push_back(*ros);
	ldout(cct, 30) << "oid_num before insert_rbd_io_stat_op is " <<  oid_num << dendl;
	oid_num += ros->oid_latency.size();
	ldout(cct, 30) << "oid_num after insert_rbd_io_stat_op is " <<  oid_num << dendl;
	delete ros;ros=NULL; 
