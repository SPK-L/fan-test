...
namespace librbd {
  ...
  struct ImageCtx {
	...
	Mutex rbd_io_stat_lock;   //protects rbd dump_ops modify
    ...
    uint32_t get_rbd_stat_io_op_stats_size();
	uint32_t get_rbd_stat_io_op_stats_oid_size();
	bool can_trim_rbd_io_stat_op(utime_t now ,double rbd_op_time_to_keep ,uint32_t max_op_size);
	void trim_rbd_io_stat_front_op();
	void insert_rbd_io_stat_op(RbdOpStat *ros);
	void set_rbd_io_stat_image_name(std::string i_name);
	void set_rbd_io_stat_pool_name(std::string p_name);
	std::string get_rbd_io_stat_image_name();
    std::string get_rbd_io_stat_pool_name();
    std::list<RbdOpStat> get_rbd_io_stat_op_stats();
