...
void AioImageRead<I>::send_request() {
  ...
  for (auto &object_extent : object_extents) {
    for (auto &extent : object_extent.second) {
	  ...
	  utime_t osd_start_time = ceph_clock_now(image_ctx.cct);
      req_comp->insert_oid_latency(pair<string, utime_t>(extent.oid.name,  osd_start_time));
      ldout(image_ctx.cct ,20) << "req_comp->oid_latency[extent.oid.name]"  << extent.oid.name << dendl;
      ...
      if (image_ctx.object_cacher) {
        ldout(cct ,20) << "TRUE image_ctx.object_cacher = " << image_ctx.object_cacher << dendl;
       ...
       ldout(cct ,20) << "FALSE image_ctx.object_cacher = " << image_ctx.object_cacher << dendl;
        req->send();
      }
    }
  }
...
void AbstractAioImageWrite<I>::send_object_requests(
  ...
  for (ObjectExtents::const_iterator p = object_extents.begin();
    ...
    utime_t osd_start_time = ceph_clock_now(image_ctx.cct);
    req_comp->insert_oid_latency(pair<string, utime_t>(p->oid.name,  osd_start_time));
    ldout(image_ctx.cct ,20) << "req_comp->oid_latency[p->oid.name]"  << p->oid << dendl;
