...
int get_extent_options(const po::variables_map &vm, uint64_t *offset,
                       uint64_t *len) {
  if (vm.count(at::OFFSET) == 0) {
    std::cerr << "rbd: must specify --offset" << std::endl;
    return -EINVAL;
  }

  if (vm.count(at::LEN) == 0) {
    std::cerr << "rbd: must specify --len" << std::endl;
    return -EINVAL;
  }

  *offset = vm[at::OFFSET].as<uint64_t>();
  *len = vm[at::LEN].as<uint64_t>();
  return 0;
}
