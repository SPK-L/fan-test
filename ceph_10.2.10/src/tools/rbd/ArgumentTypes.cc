...
void add_extent_options(boost::program_options::options_description *opt) {
  opt->add_options()
    ((OFFSET + ",o").c_str(), po::value<Extend>()->required(),
     "offset in image")
    ((LEN + ",l").c_str(), po::value<Extend>()->required(),
     "len of the data to process");
}

void add_checksum_option(boost::program_options::options_description *opt) {
  opt->add_options()
    ("checksum", po::bool_switch(), "checksum of data when read/write");
}
