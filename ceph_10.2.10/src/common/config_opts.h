 ...
 *RBD dump ops.
 */
OPTION(rbd_op_time_to_keep, OPT_DOUBLE, 60)  // dump ops keep time interval, seconds
OPTION(rbd_enable_dump_ops, OPT_BOOL, false)  // dump ops enabled
OPTION(rbd_max_dump_ops_size, OPT_U32, 60 * 10000) //dump ops max keep num 
...
//Configuration for ceph-proxy
OPTION(proxy_data, OPT_STR, "/var/lib/ceph/proxy/$cluster-$id")
OPTION(proxy_host, OPT_STR, "")
OPTION(proxy_op_timeout, OPT_DOUBLE, 0) // how many seconds to wait for a response from proxy before returning an error from a proxy operation. 0 means no limit.
OPTION(proxy_max_cache_objects, OPT_INT, 1024) //how many objects can ceph-proxy cache

//End configuration for ceph-proxy
